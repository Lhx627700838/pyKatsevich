...
def differentiate(sinogram, conf, tqdm_bar=False):
    ...
    Parameters:
    ===========
    sinogram : ndarray, shape (views, alpha, w)
    conf : dict, containing:
        - 'delta_s': spacing in lambda
        - 'pixel_span': d_alpha
        - 'pixel_height': d_w
        - 'col_coords': alpha_coords
        - 'row_coords': w_coords
        - 'SDD': source to detector distance
        - 'R0': source trajectory radius
    Returns:
    ========
    output_array : ndarray, same shape as input (views, alpha-1, w-1)
    """
...
def post_cosine_weighting(input_array, conf):
    '''
    input_array: projections after backward rebinning, size (n_proj, detector_rows, detector_columns), with detector_columns means alpha detector columns
    '''
    alpha_coords = conf['col_coords'] # alpha
    cos_alpha = np.cos(alpha_coords).reshape(1, 1, -1)
    g6 = input_array * cos_alpha
    return g6
...
def compute_hilbert_kernel(N, d_alpha, apodization='hanning'):
    ...
    Parameters:
        N: int -> kernel half-width, total size = 2N+1
        d_alpha: float -> alpha spacing (rad)
        apodization: 'cos2' or 'none'
    Returns:
        H: array of shape [2N+1], center at N
    """
    H = np.zeros(2 * N + 1)
    t_values = np.arange(-N, N + 1)

    for idx, t in enumerate(t_values):
        if t == 0:
            H[idx] = 0  # Principal value is zero
            continue
